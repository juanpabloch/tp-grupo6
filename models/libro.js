...
class Libro {
  static async estado(id) {
    ...
    let respuesta = await qy.query(query, [id]);
    return respuesta;
  }

  static async lista() {
    const query = "SELECT * FROM libro";
    let respuesta = await qy.query(query);
    return respuesta;
  }
  ...
  static async prestar(id, libro) {
    ...
    let respuesta = await qy.query(query, [id, libro]);
    return respuesta;
  }
  ...
  static async verificar(nombre, persona, categoria, id) {
    if (persona === null) {
      ...
      let respuesta = await qy.query(query, [nombre, categoria, id]);
      return respuesta;
    } else {
      const query =
        "SELECT * FROM libro WHERE nombre = ?   AND persona_id  = ?  AND categoria_id  = ? AND libro_id  = ?  ";
      let respuesta = await qy.query(query, [nombre, persona, categoria, id]);
      return respuesta;
    }
  }
  ...
  static async buscarNombre(nombre) {
    ...
    let respuesta = await qy.query(query, [nombre]);
    return respuesta;
  }
}
